...
inline void evolve(int population[640][32], int nodes[32], int tr) {             //@ _population_:_nodes_:_nodes_
    std::random_device rd;
    std::mt19937 engine(rd());
    std::uniform_int_distribution<> cutGenerator(1, 32 - 1);                                   //@ _nodes_
    std::uniform_int_distribution<> randomParent(tr * 32, (tr + 1) * 32 - 1);                 //@ _ipt_:_ipt_
    std::uniform_int_distribution<> randIndividual(0, 640 - 1);                            //@ _population_

    // conduct the crossovers
    for (int m = 0; m < 100; m++) {                                                    //@ _subGenerations_
        ...
        int offspring[32];                                                                     //@ _nodes_

        // conduct 1-point crossover
        std::copy(population[parent1], population[parent1] + crossoverCut, offspring);
        std::copy(population[parent2] + crossoverCut, population[parent2] + 32, offspring + crossoverCut);     //@ _nodes_
        ...
        if (static_cast<float>(rand()) / static_cast<float>(RAND_MAX) <= 0.3) {          //@ _mutationRate_
        ...
        for (n_i = 1; n_i < 32; n_i++) {                                                       //@ _nodes_
            ...
            for (; node_i < 32 - 1; node_i++) {                                                //@ _nodes_
                for (node_j = node_i + 1; node_j < 32; node_j++) {                             //@ _nodes_
        ...
        for (n = 1; n < 32; n++) {                                                             //@ _nodes_
            ...
            decoded[n][1] = offspring[n] >> (8 + 6);                                    //@ _yBits_:_dBits_
            decoded[n][2] = (offspring[n] << (32 - 8 - 6)) >> ((32 - 8 - 6) + 6);     //@ _yBits_:_dBits_:_yBits_:_dBits_:_dBits_
            decoded[n][3] = (offspring[n] - (((decoded[n][1] << 8) + decoded[n][2]) << 6));             //@ _yBits_:_dBits_

            // add the demand of the node to the load
            load += decoded[n][3];

            // if the capacity constraint has been violated, reset the start of the route, demand
            if (load > 100) {                                                                //@ _capacity_
        ...
        for (int r = 1; r < routes.size(); r++) {
            ...
            route[0][0] = 1348352;                                                                  //@ _depot_
            route[0][1] = 82;                                                                 //@ _depotX_
            route[0][2] = 76;                                                                 //@ _depotY_
            route[0][3] = 0;

            route[rSize - 1][0] = 1348352;                                                          //@ _depot_
            route[rSize - 1][1] = 82;                                                         //@ _depotX_
            route[rSize - 1][2] = 76;                                                         //@ _depotY_
            route[rSize - 1][3] = 0;
        ...
        int (*worst)[32] = std::max_element(population + tr * 32, population + (tr + 1) * 32, [](const int a[32], const int b[32]){return a[0] < b[0];});       //@ _nodes_:_ipt_:_ipt_:_nodes_:_nodes_
        std::copy(offspring, offspring + 32, (*worst));                                        //@ _nodes_
    }
}
...
int main(int argc, char *argv[]) {
    ...
    // read in the nodes
    int nodes[32];                                                                             //@ _nodes_
    read(nodes, 32);                                                                           //@ _nodes_

    // initialize populations based of the nodes
    int population[640][32];                                                          //@ _population_:_nodes_
    populate(population, nodes, BEST, 6, 8, 1348352, 100);                       //@ _dBits_:_yBits_:_depot_:_capacity_
    ...
    header(BEST, 784.0, time);                                                                //@ _bestKnown_
    ...
    int generation = 1;
    for (; generation <= 50000; generation++) {                                             //@ _generations_

        // shuffle the populaiton
        for (int m = 0; m < 640; m++) {                                                    //@ _population_
            int randIndex = randIndividual(engine);
            std::swap_ranges(population[m], population[m] + 32, population[randIndex]);        //@ _nodes_
        }

        // evolve the individuals in parrallel (launch the threads)
        for (int tr = 0; tr < 20; tr++) {                                                    //@ _threads_
        ...
        if (true && BEST <= 784.0) {break;}                                              //@ _optimal_:_bestKnown_

        // update the results
        if (generation % 100 == 0) {
            int (*best)[32] = std::min_element(population, population + 640, [](const int a[32], const int b[32]){return a[0] < b[0];});    //@ _nodes_:_population_:_nodes_:_nodes_
            progression.push_back(std::vector<int>(std::begin((*best)), std::end((*best))));
        }
        if (generation % 1000 == 0) {                                                            //@ _print_
            auto time = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now() - START).count();
            update(generation, BEST, 784.0, time);                                            //@ _bestKnown_
        }
    }
